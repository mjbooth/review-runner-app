...
// Public tracking URLs - completely bypasses Clerk authentication
console.log('🚀 Public tracking route loaded!');

// GET /track/[uuid] - Handle review request tracking and redirect (public access)
export async function GET(
  request: NextRequest,
  { params }: { params: { uuid: string } }
) {
  console.log('🎯 PUBLIC TRACKING ROUTE REACHED!', { uuid: params.uuid });
  ...
  try {
    ...
    logger.info('Public tracking link accessed', {
      uuid,
      userAgent,
      ipAddress,
      referer,
    ...
    if (!reviewRequest) {
      logger.warn('Invalid public tracking link accessed', { uuid, ipAddress });
    ...
    if (!reviewRequest.isActive || reviewRequest.status === 'OPTED_OUT') {
      logger.info('Inactive or opted-out public link accessed', { 
        uuid, 
    ...
    if (!reviewRequest.clickedAt) {
      await prisma.$transaction(async (tx) => {
        ...
        await tx.reviewRequest.update({
          ...
          data: {
            ...
            clickMetadata: {
              ...
              publicRoute: true, // Mark as coming from public route
            },
          },
        });
        ...
        await tx.event.create({
          data: {
            ...
            description: `Public review link clicked by ${reviewRequest.customer.firstName} ${reviewRequest.customer.lastName}`,
            metadata: {
              trackingUuid: uuid,
      ...
      logger.info('Public review request marked as clicked', {
      ...
      // Log repeat click
      logger.info('Repeat click on public review request', {
        requestId: reviewRequest.id,
      ...
      await prisma.event.create({
        data: {
          ...
          type: 'REQUEST_CLICKED',
          source: 'public_redirect',
          description: `Repeat click on public review link by ${reviewRequest.customer.firstName} ${reviewRequest.customer.lastName}`,
          metadata: {
            trackingUuid: uuid,
            repeatClick: true,
            previousClickAt: reviewRequest.clickedAt,
    ...
    // Log the redirect
    logger.info('Public route redirecting to review URL', {
      requestId: reviewRequest.id,
      redirectUrl,
    ...
    logger.error('Error processing public tracking redirect', {
      uuid: params.uuid,
...
function generateRedirectPage(businessName: string, redirectUrl: string, customerName: string): string {
  return `
    ...
    <body>
      <div class="container">
        <div class="public-badge">✅ Public Access Working</div>
        <div class="logo">⭐</div>
